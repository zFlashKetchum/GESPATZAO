...
export default function ConsultScreen({ navigation }) {
  // Definindo valores padrão para os campos de entrada
  const [codigo, setCodigo] = useState('Mesa');
  const [marca, setMarca] = useState('IKEA');
  const [cor, setCor] = useState('Madeira');
  const [numero, setNumero] = useState('12345');
  const [descricao, setDescricao] = useState('Mesa de jantar');
  const [departamento, setDepartamento] = useState('Sala de Jantar');

  return (
    <View style={styles.mainContainer}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.header}>Patrimônio</Text>
        ...
        <View style={styles.actionButtons}>
          <ActionButton label="Excluir" color="#e74c3c" icon="delete" />
          <ActionButton label="Atualizar" color="#3498db" icon="update" />
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  actionButtons: { flexDirection: 'row', justifyContent: 'space-evenly', marginTop: 20 },
  actionButton: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20 },
  buttonText: { fontSize: 18, fontWeight: 'bold', color: '#333', marginLeft: 8 },
